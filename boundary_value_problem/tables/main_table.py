...
class main_table(QtWidgets.QTableWidget):
    def __init__(self, parent = None):
        ...
        self.setHorizontalHeaderLabels(("Xi", "V(Xi)", "V2(Xi)", "V(Xi) - V2(Xi)"))
    ...
    def print_table(self, x: np.array, v: np.array, v2: np.array, s: np.array):
        self._clear_table()
        self.row_count = x.shape[0]
        self.setRowCount(self.row_count)
        for i in range(self.row_count):
            self.setItem(i, 0, self._create_cell(str(x[i])))
            self.setItem(i, 1, self._create_cell(str(v[i])))
            self.setItem(i, 2, self._create_cell(str(v2[i])))
